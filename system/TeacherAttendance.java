...
public class TeacherAttendance extends JFrame implements ActionListener {
    ...
    TeacherAttendance() {
        ...
        JLabel heading = new JLabel("Faculty Attendance");
        ...
        // Table setup
        String[] columnNames = {"Serial No.", "Employee Number", "Faculty Name", "Attendance"};
        ...
        try {
            ...
            while (rs.next()) {
                ...
                teacherEmpid.add(empid);
                model.addRow(new Object[]{serialNo++, empid, name, "Present"});
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        if (ae.getSource() == submit) {
            ...
            try {
                ...
                for (int i = 0; i < model.getRowCount(); i++) {
                    String empid = (String) model.getValueAt(i, 1);
                    String status = (String) model.getValueAt(i, 3);
                    
                    String query = "insert into teacherattendance (empid, date, status) values('"+empid+"', '"+date+"', '"+status+"')";
                    c.s.executeUpdate(query);
                }
                JOptionPane.showMessageDialog(null, " Faculty Attendance submitted successfully");
                setVisible(false);
            } catch (Exception e) {
                e.printStackTrace();
            }
